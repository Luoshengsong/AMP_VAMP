...
xhat = zeros(n,L); Xvar = zeros(n,L);
for idx = 1: n
    xn_mean = 0;
    xn_var = xvar_all(idx);
    denoiser = SparseScaEstim(CAwgnEstimIn(xn_mean,xn_var),sparsity);  % ! !
    fxnDenoise = @(rhat,rvar) denoiser.estim(rhat,rvar); 
    fxnDenoise_novar = fxnDenoise;             % novar: no variance
    [xn_temp, xn_var_temp] = denoiser.estimInit();%fxnDenoise();
    xhat(idx,:) = xn_temp; Xvar(idx,:) = xn_var_temp;
end
...
stop = false;
it = 0;
xhatNext = zeros(n,L) ;XvarNext = zeros(n,L);
% Main iteration loop
while ~stop
    ...
    for idx = 1: n
        ...
        fxnDenoise = @(rhat,rvar) denoiser.estim(rhat,rvar); 
        fxnDenoise_novar = fxnDenoise;             % novar: no variance
        [xn_temp, xn_var_temp] = fxnDenoise(rhat(idx,:), Rvar(idx,:));
        xhatNext(idx,:) = xn_temp; XvarNext(idx,:) = xn_var_temp;
    end
